...
public:
    ...
    Connection(const Attributes &attr) : m_conn(NULL)
    {
    ...
    Connection(const std::string& host, const std::string& user, const std::string& password,
               const std::string& db = "", int port = 0) : m_conn(NULL)
    {
	...
	static void result_push_back(result_t &result, const fields_t &f)
  	{
       	result.push_back(f);
    }
#endif
...
#ifdef __GXX_EXPERIMENTAL_CXX0X__
        use(boost::bind(result_push_back, boost::ref(out), _1));
#else
